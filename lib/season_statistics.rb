module SeasonStatistics
  ...
  def most_accurate_team(season)
    most_accurate_team = accuracy_by_team_season(season).max_by { |_id, accuracy| accuracy }[0]
    team_name_by_id[most_accurate_team]
  end

  def least_accurate_team(season)
    least_accurate_team = accuracy_by_team_season(season).min_by { |_id, accuracy| accuracy }[0]
    team_name_by_id[least_accurate_team]
  end

  def most_tackles(season)
    most_tackles_team_id = tackles_by_team_season(season).max_by { |_id, accuracy| accuracy }[0]
    team_name_by_id[most_tackles_team_id]
  end

  def fewest_tackles(season)
    fewest_tackles_team_id = tackles_by_team_season(season).min_by { |_id, accuracy| accuracy }[0]
